...
async function checkExistingResponse() {
    try {
        ...
        // Asumiendo que window.firebaseApp tiene un método getGuestByCode
        if (window.firebaseApp && window.firebaseApp.getGuestByCode) {
            const guestData = await window.firebaseApp.getGuestByCode(INVITATION_ID);
            
            if (guestData && guestData.status !== 'pending') {
                // Already responded, show confirmation
                const responseData = {
                    estado: guestData.status,
                    personasConfirmadas: guestData.confirmedGuests || 0,
                    nombreInvitado: guestData.name || ''
                };
    ...
    attendanceSelect.addEventListener('change', (e) => {
        if (e.target.value === 'yes') {
            ...
            setTimeout(() => {
                ...
                nameGroup.classList.add('fade-in-up');
            }, 10);
        } else if (e.target.value === 'no') {
            guestsCountGroup.style.display = 'none';
            noteGroup.style.display = 'block';
            nameGroup.style.display = 'block';
            // Name field not required for "no"
...
async function handleRSVPSubmit(event) {
    ...
    if (attendance === 'yes') {
        // Validate name (required for "yes")
        if (!name.trim()) {
            showFormError('Por favor escribe tu nombre');
            return;
        }
    ...
    try {
        ...
        // Save to Firebase using the existing function
        let result;
        if (window.firebaseApp && window.firebaseApp.updateGuestRSVP) {
            result = await window.firebaseApp.updateGuestRSVP(
                INVITATION_ID,
                responseData.estado,
                responseData.personasConfirmadas,
                name,
                note
            );
        } else {
            // Fallback: create custom save function
            result = await saveInvitationResponse(responseData);
        }
...
// Helper function to save invitation response
async function saveInvitationResponse(responseData) {
    try {
        const db = firebase.firestore();
        const responseRef = db.collection('responses').doc(INVITATION_ID);
        ...
        console.error('Error saving invitation response:', error);
        throw error;
    }
}
...
function showRSVPSuccess(responseData) {
    ...
    if (responseData.estado === 'confirmed') {
        document.getElementById('confirmation-details').textContent = 
            `¡Perfecto ${responseData.nombreInvitado}! Has confirmado asistencia para ${responseData.personasConfirmadas} persona${responseData.personasConfirmadas !== 1 ? 's' : ''}.`;
    } else {
        document.getElementById('confirmation-details').textContent = 
            `Entendido ${responseData.nombreInvitado || ''}. Lamentamos que no puedas asistir, pero agradecemos tu respuesta.`;
    }
